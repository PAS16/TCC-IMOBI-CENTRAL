...
if (isset($_POST['acao'])) {
    if ($_POST['acao'] === 'cadastrar') {
        ...
        if ($nome && $cpf && $email) {
            $stmt = $conn->prepare("INSERT INTO CLIENTE (nome, cpf, telefone, email) VALUES (?, ?, ?, ?)");
    ...
    if ($_POST['acao'] === 'editar') {
        ...
        if ($id && $nome && $cpf && $email) {
            $stmt = $conn->prepare("UPDATE CLIENTE SET nome=?, cpf=?, telefone=?, email=? WHERE idCLIENTE=?");
...
$sql = "SELECT * FROM CLIENTE $where ORDER BY idCLIENTE ASC";
...
<div class="fade-in-page w-full px-4 py-10 flex flex-col items-center">
  <div class="bg-[#1f1f2f]/90 backdrop-blur-md p-8 rounded-3xl w-full max-w-6xl shadow-2xl space-y-6 card-dynamic border border-[#2a2a3f]/50">
    ...
    <h2 class="text-3xl font-bold tracking-wide text-center text-gray-200 title-glow">Gerenciamento de Clientes</h2>
      ...
      <table class="w-full text-left border-collapse">
        ...
        <?php while($linha = $resultado->fetch_assoc()): ?>
          <tr class="bg-[#1f1f2f] hover:bg-[#2c2c44] transition">
            <td class="p-3 border border-[#2a2a3f]"><?= $linha['idCLIENTE'] ?></td>
            ...
            <td class="p-3 border border-[#2a2a3f] flex gap-2">
              <button onclick="abrirModal('editar', <?= $linha['idCLIENTE'] ?>, '<?= addslashes($linha['nome']) ?>', '<?= addslashes($linha['cpf']) ?>', '<?= addslashes($linha['telefone']) ?>', '<?= addslashes($linha['email']) ?>')" class="px-3 py-1 rounded-md font-bold btn-glow shadow-md">Editar</button>
              <button onclick="abrirModal('excluir', <?= $linha['idCLIENTE'] ?>)" class="px-3 py-1 rounded-md font-bold btn-glow shadow-md">Excluir</button>
            </td>
          </tr>
        <?php endwhile; ?>
        </tbody>
...
function abrirModal(tipo, id='', nome='', cpf='', telefone='', email='') {
    ...
    if(tipo==='cadastrar') {
        ...
        $('#modal-body').html(`
            ...
            <input type="text" id="cpf" placeholder="CPF" class="w-full p-2 mb-2 rounded bg-[#2a2a3f]">
        ...
        $('#modal-body').html(`
            ...
            <input type="text" id="cpf" value="${cpf}" class="w-full p-2 mb-2 rounded bg-[#2a2a3f]">
        ...
        $('#modal-body').html('<p>Deseja realmente excluir este cliente?</p>');
        $('#modal-confirm').off('click').click(function(){
